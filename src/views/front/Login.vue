...
<div class="main">
  <form>
    ...
    <input class=input_1  type="password" size=15 name="password" placeholder=密码 v-model="password"><br />
        ...
        if (this.power === "users") {
          request({
            ...
            if (data.data !== "用户名或密码错误") {
              _this.$router.push("/")
              _this.$store.commit("changeUserId",data.data.id)
              _this.$store.commit("changePU", json)
              if (data.data.avatar !== "") {
                _this.$store.commit("changeAvatarUrl",data.data.avatar)
              } else {
                _this.$store.commit("changeAvatarUrl","img/home/personal.png")
              }
            } else {
              alert(data.data)
            }
          ...
          request({
            ...
            if (data.data !== "用户名或密码错误") {
              _this.$store.commit("changeUserId",data.data.id)
              _this.$store.commit("changePU", json)
              _this.$store.commit("changeRoleId", data.data.roleId)
              _this.$router.replace("/backGround/welcome")
            } else {
              alert(data.data)
            }
          })
